...
The runtime pauses before an action only when the actor marks it as dangerous. The actor is instructed to mark irreversible or financial click and type actions, but the current runtime does not independently classify every high-impact action. Review the proposed action and the visible page state before confirming. Do not treat confirmation as permission to automate actions that violate a website's rules or a user's expectations.
...
- A model can omit the dangerous-action marker, so human review remains necessary for every consequential step.
